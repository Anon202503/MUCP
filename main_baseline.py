...
if __name__ == '__main__':
    ...
    data_train, data_test, _ = utils.get_dataset(
        root=args.data_dir,
        data_name=args.data_name,
        model_name=args.model_name,
    )
    train_dl = DataLoader(data_train, batch_size=batch_size, shuffle=True, num_workers=0)
    test_dl = DataLoader(data_test, batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        train_loss, train_acc = train_loop(train_dl, model, criterion, optimizer, device)
        test_loss, test_acc = test_loop(test_dl, model, criterion, device)
