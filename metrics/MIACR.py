...
def SVC_fit_predict_cp(shadow_train, shadow_test, target_train, target_test, cal_ds, alpha):
    ...
    q_hat = find_quantile(cal_scores, n_cal, alpha)
...
def SVC_MIA(shadow_train, shadow_test, target_train, target_test, cal_dl, model, device, alpha):
    ...
    acc_conf = SVC_fit_predict_cp(
        shadow_train_conf, shadow_test_conf, target_train_conf, target_test_conf, cal_ds_conf, alpha
    )
    return acc_conf
